...
class CounselorFormList extends Component
{
    public $user;
    ...
    {
        $data = Form::where('counselor_id', $this->user->counselor->id)->latest()->get();

        return view('livewire.backend.admin.user.counselor-form-list', [
            'data' => $data,
        ])
    ...
    public function statusClick($id, $newStatus)
    {
        ...
        return redirect()->route('admin.user.counselor.forms', $this->user->id)->with('success', 'Form is successfully status!');
    }
    ...
    public function delete()
    {
        try{
            ...
            return redirect()->route('admin.user.counselor.forms', $this->user->id)->with('success', 'Form is successfully deleted!');
        }catch(\Exception $e){
